...
import {EventProps, ResultProps} from "@/types/index.d";
...
const renderRow = (item: EventProps) =>
    (
        <tr key={item.id} className='py-4 border-gray-200 even:bg-slate-50 text-sm hover:bg-lamaPurpleLight'>
            ...
            <td className='hidden md:table-cell font-bold'>{item.startTime}</td>
            <td className='hidden md:table-cell font-bold'>{item.endTime}</td>
            <td>
...
const TeacherListPage = () =>
    (
        <div className='mx-4'>
            <Card className='p-4'>
                <TableNavigation title={'All events'}/>
                {/*LIST*/}
